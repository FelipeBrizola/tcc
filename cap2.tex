...
% Sergio: incluir as seções abaixo enumeradas. Não é necessário que sejam demasiadamente longas, porém são importantes para alinhar o leitor:
...
% As seções abaixo (CoAP, Rest, MQTT, fora os trabalhos relacionados) podem fazer parte de uma só seção. Ou seja, viram subseções de uma seção chamada "Protocolos relacionados".

\section{BGP}
% Sergio: não sei se incluir o BGP aqui é relevante. Talvez faz como conversamos na reunião, se existe alguma semelhança do teu protocolo com BGP, joga isso pra outra seção e referencia ou explica um pouco do BGP lá.
