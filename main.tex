...
%   \tipotrabalho{\tci}         % Trabalho de Conclusão I
  ...
  %----------------------------------------------------------------
  \orientador{Dr. Sérgio Johann Filho}
  ...
  % (OBRIGATÓRIO, ao menos para teses e dissertações)
  %----------------------------------------------------------------
  \begin{abstract}{resource mapping, fog computing, internet of things, mapping protocol}

    Fog computing can be seen as a paradigm that provides storage, computing, and network services between end devices and servers in the cloud.
    From this paradigm comes the \textit{fog node} concept, which ranges from end-devices with low computational capacity to powerful servers in the cloud.
    Commonly a fog node manages resources such as sensors and actuators.
    These features communicate with the fog node through the CoAP protocol.
    Thus, a node interacts exclusively with its resources because it is unaware of the sensors and actuators managed by its network neighbors.

    The problems related to standardization, discovery and synchronization are the motivators of this project, since there are currently no mechanisms in which a fog node maps the available resources and disseminates their resources on the network.

    This project will present the modeling and implementation of a protocol with low transmission cost, distributed, adaptive and simple to be used in already implemented network topologies.
    Finally, test scenarios and case studies will be presented with the objective of validating the proposed solution.
  ...
  %----------------------------------------------------------------
  \include{cap1} %intro
  \include{cap2} %ref teorico. estado da arte
  \include{cap3} %proposta
  \include{cap4} %como foi feito e metodologia experimental
